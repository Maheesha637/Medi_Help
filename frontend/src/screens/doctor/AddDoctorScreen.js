...
const AddDoctorScreen = ({ navigation }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert('Error', error.response?.data?.message || 'Failed to add doctor');
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        ...
        >
          <View style={styles.formCard}>
            ...
            <View style={styles.daysSection}>
              ...
              <View style={styles.daysContainer}>
                {DAYS.map((day) => (
                  ...
                  >
                    <Text style={[styles.dayChipText, availableDays.includes(day) && styles.dayChipTextActive]}>
                      {day.substring(0, 3)}
                    </Text>
                  </TouchableOpacity>
                ))}
            ...
            <CustomInput label="Bio" value={bio} onChangeText={setBio} placeholder="Short description about the doctor" multiline={true} numberOfLines={3} icon="information-circle-outline" />

            <CustomButton title="Add Doctor" onPress={handleSubmit} loading={loading} style={{ marginTop: 8 }} />
          </View>
        </ScrollView>
      </KeyboardAvoidingView>
    </SafeAreaView>
